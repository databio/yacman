...
class YAMLConfigManager(dict):
    """
    A class that provides the yaml configuration manager benefits, but without the 
    requirement of attmap (and without providing the attribute-style access).
    ...
    def __init__(
        ...
        if filepath:
            ...
            setattr(getattr(self, IK), WAIT_MAX_KEY, wait_max)
            setattr(getattr(self, IK), FILEPATH_KEY, mkabs(filepath))
            setattr(getattr(self, IK), RO_KEY, not writable)

        getattr(self, IK)[WRITE_VALIDATE_KEY]=write_validate
        ...
        getattr(self, IK)["expand"] = expand


    def __del__(self):
        if hasattr(getattr(self, IK), FILEPATH_KEY) and not getattr(getattr(self, IK), RO_KEY, True):
            self.make_readonly()
    ...
    def __enter__(self):
        setattr(getattr(self, IK), ORI_STATE_KEY, getattr(getattr(self, IK), RO_KEY, True))
        if not getattr(getattr(self, IK), RO_KEY, None):
            return self
        else:
            self.make_writable()
            return self
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        self.write()
        if getattr(getattr(self, IK), ORI_STATE_KEY, False):
            self.make_readonly()
    ...
    def validate(self, schema=None, exclude_case=False):
        ...
        try:
            _validate(
                self.to_dict(expand=True), schema or getattr(getattr(self, IK), SCHEMA_KEY)
            )
        except ValidationError as e:
            _LOGGER.error(
                f"{self.__class__.__name__} object did not pass schema validation"
            )
            if getattr(getattr(self, IK), FILEPATH_KEY, None) is not None:
    ...
    def write(self, filepath=None, schema=None, exclude_case=False):
        ...
        """
        if getattr(getattr(self, IK), RO_KEY, False) and filepath is None:
            raise OSError(
                "You can't call write on an object that was created in read-only mode."
            )
        if schema is not None or getattr(getattr(self, IK), WRITE_VALIDATE_KEY):
            self.validate(schema=schema, exclude_case=exclude_case)
        filepath = _check_filepath(filepath or getattr(getattr(self, IK), FILEPATH_KEY, None))
        ...
        if filepath != getattr(self[IK], FILEPATH_KEY, None):
            ...
            if getattr(getattr(self, IK), FILEPATH_KEY, None):
                self.make_readonly()
            setattr(getattr(self, IK), FILEPATH_KEY, filepath)
            create_lock(filepath, getattr(getattr(self, IK), WAIT_MAX_KEY, DEFAULT_WAIT_TIME))
        setattr(getattr(self, IK), RO_KEY, False)
    ...
    def make_readonly(self):
        ...
        """
        if self._remove_lock(getattr(getattr(self, IK), FILEPATH_KEY, None)):
            setattr(getattr(self, IK), RO_KEY, True)
    ...
    def make_writable(self, filepath=None):
        ...
        """
        if not getattr(getattr(self, IK), RO_KEY, True):
            _LOGGER.info(
                "Object is already writable, path: {}".format(
                    getattr(getattr(self, IK), FILEPATH_KEY, None)
                )
            )
            return self
        ori_fp = getattr(getattr(self, IK), FILEPATH_KEY, None)
    ...
    def file_path(self):
        ...
        return getattr(getattr(self, IK), FILEPATH_KEY, None)
    ...
    def writable(self):
        ...
        """
        _warn_deprecated(obj=self)
        attr = getattr(getattr(self, IK), RO_KEY, None)
    ...
    def __getitem__(self, item, expand=False):
        ...
        return _safely_expand_path(v) if getattr(self, IK)["expand"] or expand else v
